    ...
    return (
        <div>
            <div className='Filter'>
                ...
                <button className='Filter-add' onClick={() => navigate(routes.ADDCATEGORIES)}>+ Добавить товар</button>
            </div>
            <ul className="list">
                    ...
                    {categories && categories?.map((e: any, i: any) => (
                        <li className='list-item'>
                            ...
                            <ul ref={x} className='list-item-drop ' style={tr == e?.id ? { display: "inline-block", zIndex: 10 } : { display: "none", zIndex: 0 }} >
                                <li className='list-item-drop-text'><Link to={routes.UPDATECATEGORIES + `/${e?.id}`}>Изменить</Link></li>
